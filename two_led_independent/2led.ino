...
const char *willTopic = "connected/19216812312";
const char *willMessage = "0";
boolean willRetain = false;

const char *device_serial_number = "19216812312";
const bool ON = true;
const bool OFF = false;
const int button = 15;   //khai bao button reset
const int ledwifi = 25;  //khai bao den led wifis
const int ledreset = 2; //khai bao den led wifi
// const int outputpin = 33; //Khai bao chan doc nhiet do
const int Relay = 4; //Khai bao chan output vao relay
const int l_m1 = 19;  // define pin led mode
const int l_m2 = 21;
const int l_m3 = 33;
const int l_m4 = 23;

const int replay[3] = {14, 27, 26};
...
int solan = 0;
int value = 0;
// int sensorPin = 5;// chân analog kết nối tới cảm biến LM35
...
long cy_run_time1 = 0;
long cy_rest_time1 = 0;
long cy_apply_datetime1 = 0;
long cy_stop_datetime1 = 0;

long cy_repeat = 0;

int rt_repeat = 0;

const char *AP_Name = "sky1023";
String mon;
String tue;
String wed;
String thu;
...
AlarmID_t My_cy_timer_start1;
AlarmID_t My_cy_timer_stop1;
AlarmID_t My_rt_timer_start1;
AlarmID_t My_rt_timer_repeat1;
...
WiFiManager wifiManager; //set wifi manager
...
void setup()
{
    ...
    pinMode(button, INPUT_PULLDOWN); //Khai bao button
    pinMode(ledwifi, OUTPUT);        //Khai bao led wifi
    pinMode(ledreset, OUTPUT);
//out put Pin relay 

    pinMode(replay[0], OUTPUT);
    pinMode(replay[1], OUTPUT);
    pinMode(replay[2], OUTPUT);
...
void loop()
{
    ...
    if (now - lastMsg > 3000)
    {
        ...
        // celsius = nearest / 10;
        // // cai dat va tinh toan nhiet do cho cam bien ngoai
        // celsius = ((temprature_sens_read() - 32) / 1.8);
        // // cai dat va tinh toan nhiet do cho internal sensor cua esp32
...
void stop_all_Alarm_funtion()
{
    ...
    Alarm.free(My_cy_timer_start1);
    Alarm.free(My_cy_timer_stop1);
    Alarm.free(My_rt_timer_start1);
    Alarm.free(My_rt_timer_repeat1);
...
void mqttCallback(char *topic, byte *payload, unsigned int length)
{
    ...
    if ((String(topic).startsWith("setting")) && (payload[0] == '{'))
    {
        ...
        case MODE_CYCLE:
        {
            ...
            Serial.println("da bat mode 3 cua replay 1");
            if(relayNo == 1){
            ...
            if(relayNo == 2){
              cy_run_time = root["setting"]["run_time1"];
              cy_rest_time = root["setting"]["rest_time1"];
              cy_apply_datetime1 = root["setting"]["apply_datetime1"];
              cy_stop_datetime1 = root["setting"]["stop_datetime1"];
              cy_repeat = root["setting"]["repeat1"];

              if ((cy_apply_datetime1 - time(nullptr)) < 0)
              {
                  Run_Cy_Alarm();
              }
              else
              {
                  My_cy_timer_start = Alarm.timerOnce(cy_apply_datetime1 - time(nullptr), Run_Cy_Alarm); // hen gio BAT  che do cycle time
              }
              My_cy_timer_stop = Alarm.timerOnce(cy_stop_datetime1 - time(nullptr), StopcycleMode); // hen gio TAT che do cycle time
              Serial.println(time(nullptr));
              is_set_alarm_cycle = false;
              Serial.println("bat che do CYCLE TIME");
...
void led_mode(int l1, int l2, int l3, int l4, int a, int b, int c, int d)
{ //function led mode
